		...
		template(v-for='(group, index) in groups')
			fieldset(:is='tag', :class='getFieldRowClasses(group)')
				legend(v-if='group.legend') {{ group.legend }}
					span.glyphicon(v-if='isToggleCollapse' class='pull-right' :class='collapseClass' @click='toggleCollapse' data-toggle='collapse' :data-target='getTarget(index)' aria-expanded="false" :aria-controls='getTarget(index)')

				.collapse(:id='getElementName(index)', class='in')
...
export default {
	...
	data() {
		return {
			...
			collapseClass: "glyphicon-menu-down"
		};
	},
	...
	methods: {
		getElementName(index) {
			return "collapseGroup" + index;
		},
		getTarget(index) {
			return "#" + this.getElementName(index);
		},
		toggleCollapse(e) {
			let groupName = $(e.target).data("target");
			if ($(groupName).hasClass("in")) {
				this.collapseClass = "glyphicon-menu-up";
				//$('[data-target="' + groupName + '"').removeClass('glyphicon-menu-down').addClass('glyphicon-menu-up')
			} else {
				this.collapseClass = "glyphicon-menu-down";
				//$('[data-target="' + groupName + '"').removeClass('glyphicon-menu-up').addClass('glyphicon-menu-down')
			}
		},
